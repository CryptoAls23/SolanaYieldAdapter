...
#[derive(Accounts)]
pub struct DeprecateAdapter<'info> {
    #[account(
        mut,
...
pub fn handler(ctx: Context<DeprecateAdapter>, reason: String) -> Result<()> {
    require!(reason.len() <= 128, RegistryError::ReasonTooLong);

    let entry = &mut ctx.accounts.adapter_entry;
    require!(entry.status == status::ACTIVE, RegistryError::NotActive);
    ...
    msg!("Adapter deprecated: {}, reason: {}", entry.adapter_program, reason);
    Ok(())
}
